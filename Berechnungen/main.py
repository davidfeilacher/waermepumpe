...
E_gemessen={"10.3.2020 bis 31.3.2020" : 2123, "04 2020" : 1600, "05 2020" : 987, "06 2020" : 562, \
...
T7,T14,T19, Tav = T7_22, T14_22, T19_22, Tav_22
...
daysPerMonth=daysPerMonth_20

hoursPerMonth=[value*24 for value in daysPerMonth]
hoursPerMonthCS = np.cumsum(hoursPerMonth).tolist()
monthindex=0

for i in range(len(T_ip_av)):
    Pw,Pel = data_interpol.PowerRequirement(T_ip_av[i])
...
EWärmeSumMonth =[int(value) for value in EWärmeSumMonth]      
print (EWärmeSumMonth, int(EWärmeSum))
